...
def test_active_library_can_validate_and_rebuild(
    ...
    expect(rebuild_dialog.locator("input[type=checkbox]:checked")).to_have_count(8)
    rebuild_dialog.get_by_role("button", name="Clear all").click()
    expect(rebuild_dialog.get_by_role("button", name="Rebuild 0 selected")).to_be_disabled()
    rebuild_dialog.get_by_role("checkbox", name="Titles", exact=False).check()
    rebuild_dialog.get_by_role("checkbox", name="Agent guide", exact=False).check()
    rebuild_dialog.get_by_role("button", name="Rebuild 2 selected").click()
    expect(page.get_by_role("status")).to_contain_text("Completed 2 selected rebuild tasks")
    assert "Journal Article" in (library / "indexes" / "titles.md").read_text(encoding="utf-8")
    assert (library / "AGENTS.md").is_file()
    assert not (library / "indexes" / "authors.md").exists()
    assert not (library / ".gitignore").exists()
